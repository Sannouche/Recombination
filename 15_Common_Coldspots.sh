...
# Usage: ./intersect_coldspot_categories.sh /path/to/folder
...
for category in 1Mb 40kb whole_chr; do
  ...
  # Collect file groups
  fw_files=($(ls "$folder"/Freshwater*_coldspots_"${category}".bed_merged.bed 2>/dev/null))
  sw_files=($(ls "$folder"/Saltwater*_coldspots_"${category}".bed_merged.bed 2>/dev/null))
  ...
  if [[ ${#fw_files[@]} -gt 1 ]]; then
    ...
    intersect_all "${fw_files[@]}" > "$folder/coldspots_${category}_FW_common.bed"
  fi

  # Saltwater
  if [[ ${#sw_files[@]} -gt 1 ]]; then
    echo "?? Intersecting Saltwater files..."
    intersect_all "${sw_files[@]}" > "$folder/coldspots_${category}_SW_common.bed"
  fi

  # All
  if [[ ${#all_files[@]} -gt 1 ]]; then
    echo "?? Intersecting All files..."
    intersect_all "${all_files[@]}" > "$folder/coldspots_${category}_ALL_common.bed"
  fi
