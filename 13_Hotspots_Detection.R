...
detect_hotspots <- function(data) {
  ...
  # Create empty lists to collect hotspot rows
  hotspots_40kb <- list()
  hotspots_1Mb <- list()
  hotspots_whole_chr <- list()
 ...
 # Check thresholds and add rows to respective hotspot lists
  ...
  return(list(
    hotspots_40kb = rbindlist(hotspots_40kb),
    hotspots_1Mb = rbindlist(hotspots_1Mb),
    hotspots_whole_chromosome = rbindlist(hotspots_whole_chr)
  ))
}


# Step 1: Detect hotspots
hotspots <- detect_hotspots(recomb_data)
...
# Step 3: Save the hotspots to the output file
fwrite(hotspots$hotspots_40kb, file.path(paste0(file_name, "_hotspots_40kb.txt")), sep = "\t")
fwrite(hotspots$hotspots_1Mb, file.path(paste0(file_name, "_hotspots_1Mb.txt")), sep = "\t")
fwrite(hotspots$hotspots_whole_chromosome, file.path(paste0(file_name, "_hotspots_whole_chr.txt")), sep = "\t")

# Optional: Print the output file name
cat("Hotspots saved to:", output_file, "\n")
